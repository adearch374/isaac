...
def get_assignment_file(upload):
    ...
    upload.stream.seek(0)
    valid_header = ((extension == 'pdf' and header.startswith(b'%PDF')) or
...
def add_assignment():
    ...
    if request.method == 'POST':
        try:
            ...
            max_score = int(request.form.get('max_score', '100')); class_id = int(request.form.get('class_id')); subject_id = int(request.form.get('subject_id'))
        except (TypeError, ValueError):
            flash('Please provide valid class, subject, due date, and score values.', 'error'); return redirect(url_for('add_assignment'))
        if max_score < 1 or not teacher_has_assignment_access(current_user.id, class_id, subject_id):
            flash('You can only assign approved subjects to approved classes, with a valid maximum score.', 'error'); return redirect(url_for('add_assignment'))
...
def grade_submission(submission_id):
    ...
    if submission.assignment.teacher_id != current_user.id:
        flash('Access denied', 'error'); return redirect(url_for('teacher_assignments'))
    try: score = int(request.form.get('score', ''))
    except ValueError:
        flash('Score must be a whole number.', 'error'); return redirect(url_for('teacher_assignment_submissions', assignment_id=submission.assignment_id))
    if score < 0 or score > submission.assignment.max_score:
        flash(f'Score must be between 0 and {submission.assignment.max_score}.', 'error'); return redirect(url_for('teacher_assignment_submissions', assignment_id=submission.assignment_id))
    submission.score = score; submission.feedback = request.form.get('feedback', '').strip(); submission.graded_at = datetime.utcnow(); submission.status = 'graded'
    db.session.commit(); flash('Submission graded successfully.', 'success'); return redirect(url_for('teacher_assignment_submissions', assignment_id=submission.assignment_id))
...
def edit_assignment(assignment_id):
    ...
    if assignment.submissions:
        flash('Assignments with submissions cannot be edited.', 'error'); return redirect(url_for('teacher_assignment_submissions', assignment_id=assignment.id))
    if request.method == 'POST':
        try: assignment.due_date = datetime.strptime(request.form.get('due_date', ''), '%Y-%m-%dT%H:%M'); assignment.max_score = int(request.form.get('max_score', '100'))
        except (TypeError, ValueError): flash('Please provide a valid due date and maximum score.', 'error'); return redirect(url_for('edit_assignment', assignment_id=assignment.id))
        assignment.title = request.form.get('title', '').strip(); assignment.description = request.form.get('description', '').strip()
        if not assignment.title or not assignment.description or assignment.max_score < 1: flash('Title, instructions, and a valid maximum score are required.', 'error'); return redirect(url_for('edit_assignment', assignment_id=assignment.id))
        db.session.commit(); flash('Assignment updated successfully.', 'success'); return redirect(url_for('teacher_assignments'))
...
def delete_assignment(assignment_id):
    if current_user.role != 'teacher':
        flash('Access denied', 'error'); return redirect(url_for('index'))
    assignment = Assignment.query.filter_by(id=assignment_id, teacher_id=current_user.id).first_or_404()
    if assignment.submissions: flash('Assignments with submissions cannot be deleted.', 'error'); return redirect(url_for('teacher_assignment_submissions', assignment_id=assignment.id))
    db.session.delete(assignment); db.session.commit(); flash('Assignment deleted successfully.', 'success'); return redirect(url_for('teacher_assignments'))
...
def student_assignments():
    if current_user.role != 'student':
        flash('Access denied', 'error'); return redirect(url_for('index'))
    student = Student.query.get_or_404(current_user.id); assignments = Assignment.query.filter_by(class_id=student.class_id).order_by(Assignment.due_date.asc()).all()
...
def assignment_file(filename):
    ...
    if (current_user.role == 'teacher' and submission.assignment.teacher_id != current_user.id) or (current_user.role == 'student' and submission.student_id != current_user.id): return redirect(url_for('index'))
